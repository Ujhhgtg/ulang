    ...
    match cc {
        Cc::Cosmocc => {
            if let Err(msg) = codegen.compile_to_object_for_triple(
                ...
                error::emit_error_opt(&codegen.source, &codegen.path, None, "codegen error", &msg);
                process::exit(1);
            }
            ...
            if let Err(msg) = codegen::CodeGen::link_executable(
                ...
                error::emit_error_opt(&codegen.source, &codegen.path, None, "link error", &msg);
                let _ = std::fs::remove_dir_all(&aarch64_dir);
                let _ = fs::remove_file(&obj_path);
                process::exit(1);
            }
    ...
    match mode {
        Mode::Run { opt } => {
            ...
            let mut codegen = match codegen::CodeGen::new_jit(
                ...
                Ok(cg) => cg,
                Err(e) => {
                    error::emit_error_opt(&source, &path, None, "codegen error", &e);
                    process::exit(1);
                }
            };
            codegen.overloads = overloads;
            if let Err(e) = codegen.jit_run(&program) {
                error::emit_error_opt(&source, &path, None, "runtime error", &e);
                process::exit(1);
            }
